...
test('uses editor elimination bounds when a local draft supplies them', () => {
  const room = new GameRoom({
    tileSize: 48,
    colliders: [],
    spawns: [{ x: 100, y: 570, gravity: 1, speedX: 0 }],
    elimination: { top: -120, bottom: 620, leftMargin: 60 }
  });
  room.join('a');
  room.start('a');
  room.tick(1 / 40);
...
test('gives a substantially larger but continuously fading catch-up bonus farther from centre', () => {
  ...
  assert.equal(farBonus > 150, true);
  assert.equal(farBonus > nearBonus, true);
